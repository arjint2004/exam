...
(function($) {
    //------------------------------------------------------------------------------
    // Provide some site wide defaults
    //------------------------------------------------------------------------------
    $.fn.snsDataTable = function(settings) {
        settings.bPaginate       = settings.bPaginate       || true
        settings.sPaginationType = settings.sPaginationType || "full_numbers"
        settings.bLengthChange   = settings.bLengthChange   || true
        settings.bSortClasses    = settings.bSortClasses    || false
        settings.bAutoWidth      = settings.bAutoWidth      || false
        settings.sScrollY        = settings.sScrollY        || '200px'
        //settings.sScrollY        = settings.sScrollY      || '315px'
        settings.sDom            = settings.sDom            || 'rt<"bottom"flp>'
        settings.iDisplayLength  = settings.iDisplayLength  || 17,
        settings.aLengthMenu     = [ settings.iDisplayLength, 50, 100, 200 ],
        this.dataTable(settings)

        // if the page has divs to receive dataTable controls, move the controls
        $('#dataTableSearch').html($('.dataTables_filter'))

        return this
    }
    //------------------------------------------------------------------------------
    // Adds a sort column to a dataTable.
    //------------------------------------------------------------------------------
    $.fn.selectableDataTable = function(settings) {
        settings.sSelectionControl = settings.sSelectionControl || 'checkbox'

        var dt = this

        // find the maximum 'th' rowspan so our inserted header can match it
        var rowspan = 1
        var headers = $('th',dt).each(function() { rowspan = Math.max(rowspan, this.rowSpan) })

        // add the header for the sort column, making it a 'select all' button if we're not using radio buttons
        var sortheader = $("<th></th>")
        sortheader.attr('rowspan', rowspan).addClass('selectColumn')
        if (settings.sSelectionControl == 'checkbox') {
           sortheader.attr('title', 'Select All')
                     .addClass('selectColumnButton')
                     .click(function(event) { dtSelectAll(dt,this) })
        }
        headers.first().before(sortheader)

        // create function which prepend a sort column to a table row
        var sortColumnTemplate = "<td style='text-align:center; width: 24px;'><input type='" + settings.sSelectionControl + "' name='selectrow'/></td>"
        dt.insertSortColumn = function(i,row) {
            var selectColumn = $(sortColumnTemplate).addClass('selectColumn')
            $(row).click(function(event) { dtOnRowClick(dt,event) })
            .find('td').first().before(selectColumn)
            return row
        }

        // insert the sort column
        $('tr', dt).each(dt.insertSortColumn)
        // give others the ability to add rows which automatically acquire a sort column
        dt.addRow = function(columndata) {
            dt.fnAddData(columndata)
            var rows = dt.fnGetNodes()
            return dt.insertSortColumn(0,rows[rows.length-1])
        }

        if (!settings.aoColumnDefs)
            settings.aoColumnDefs = []
        settings.aoColumnDefs.push({ aTargets: [0], bSortable: false })

        dt = this.snsDataTable(settings).addClass('tableSelectable')
...
var dtPreviousSelection = null
function dtSelectRow(row,sSelectionControl,select) {
    row = $(row)
    if (select == undefined)
        select = !row.hasClass('row_selected')
    row[select ? 'addClass' : 'removeClass']('row_selected')
    var input = row.find('input[type="' + sSelectionControl + '"]')
    if (input) {
        input.attr('checked', select)
    }
    return select
}
...
function dtOnRowClick(dataTable,event) {
    ...
    if (target.type == undefined
     || target.type == settings.sSelectionControl) {
       if (settings.sSelectionControl == 'radio' && dataTable.prevSelected) {
            dtSelectRow(dataTable.prevSelected, settings.sSelectionControl, false)
       }
       dataTable.prevSelected = event.currentTarget
       dtSelectRow(event.currentTarget, settings.sSelectionControl)
       if (dataTable.fnSelectionChangedCallback) {
           dataTable.fnSelectionChangedCallback(this)
       }
    }
}
function dtSelectAll(dataTable,button) {
    button.checked = !button.checked
    $(button)[button.checked ? 'addClass' : 'removeClass']('selected')
    $(dataTable.fnGetNodes()).each( function (i,node) { 
       dtSelectRow(node, dataTable.fnSettings().sSelectionControl, button.checked)
    });
    if (dataTable.fnSelectionChangedCallback)
        dataTable.fnSelectionChangedCallback(this)
}
...
(function($, window, document) {
	...
	_oExt.oPagination = {
	     ...
	     *           node:nPaging - the DIV which contains this pagination control
            ...
            nFirst.title = "First Page";
            nPrevious.title = "Previous Page";
            nNext.title = "Next Page";
            nLast.title = "Last Page";
            ...
            //XXX if (iPages <= 1)
            //XXX     $(an).addClass('paging_superfluous')
            //XXX else
            //XXX     $(an).removeClass('paging_superfluous')
	...
	$.fn.dataTable = function( oInit )
	{
		...
		function classSettings ()
		{
			 ...
			 */
			this.aLengthMenu = [ 10, 50, 100, 200 ];
		...
		function _fnDraw( oSettings )
		{
			...
			else
			{
				...
				nTd.colSpan = _fnVisbleColumns( oSettings ) + 1 // horrible hack
		...
		function _fnFeatureHtmlTable ( oSettings )
		{
			...
			if ( oSettings.oScroll.bAutoCss )
			{
				...
				nScrollBody.style.overflowY = "auto";
			}
		...
		return this.each(function()
		{
            ...
            _fnMap( oSettings, oInit, "sSelectionControl" );
